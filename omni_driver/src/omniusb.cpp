...
int OmniUsb::calibrationStyle = 0;

OmniUsb::OmniUsb(const std::string &name, const std::string &path_urdf, const std::string &path_srdf) :
...
void OmniUsb::getJointAnglesFromDriver()
{
...
void OmniUsb::autoCalibration()
{
    ...
    {
        OmniUsb::calibrationStyle = HD_CALIBRATION_ENCODER_RESET;
        ROS_INFO("HD_CALIBRATION_ENCODER_RESET..");
    }
    if (supportedCalibrationStyles & HD_CALIBRATION_INKWELL)
    {
        OmniUsb::calibrationStyle = HD_CALIBRATION_INKWELL;
        ROS_INFO("HD_CALIBRATION_INKWELL..");
    }
    if (supportedCalibrationStyles & HD_CALIBRATION_AUTO)
    {
        OmniUsb::calibrationStyle = HD_CALIBRATION_AUTO;
        ROS_INFO("HD_CALIBRATION_AUTO..");
    }

    if (OmniUsb::calibrationStyle == HD_CALIBRATION_ENCODER_RESET)
    {
        do
        {
            hdUpdateCalibration(OmniUsb::calibrationStyle);
...
/*######################################################################################*/
    ...
    autoCalibration();
    handle_callback = hdScheduleAsynchronous(&OmniUsb::callback, this, HD_MAX_SCHEDULER_PRIORITY);
    state.connected = true;
    return 1;
}

bool OmniUsb::connected()
{
...
HDCallbackCode OmniUsb::callback(void *pdata)
{
    OmniUsb *omni = static_cast<OmniUsb*>(pdata);
