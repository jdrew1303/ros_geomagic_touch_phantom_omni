...
private:
     ...
     * @param pdata Receives a pointer to an OmniUsb object.
...
public:
    ...
    /**
     * @brief OmniUsb constructor, calls OmniBase constructor with same parameter.
     * @param name Reference to string of omni name.
     */
    OmniUsb(const std::string &name, const std::string &path_urdf, const std::string &path_srdf);
    ~OmniUsb();
...
typedef boost::shared_ptr<OmniUsb> OmniUsbPtr;